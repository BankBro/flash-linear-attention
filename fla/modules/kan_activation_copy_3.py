...
class RangeTracker():
    ...
    def _plot_single_kan_activation(self, ax, kan_activation, name, color):
        """绘制单个KAN激活函数"""
        try:
            # 使用grid_range作为采样区间，并扩大0.5倍
            x_min, x_max = kan_activation.grid_range
            range_width = x_max - x_min
            x_min_extended = x_min - 0.5 * range_width
            x_max_extended = x_max + 0.5 * range_width
            x_plot = torch.linspace(x_min_extended, x_max_extended, 512, device=kan_activation.grid.device)
            x_plot = x_plot.unsqueeze(0).unsqueeze(-1)  # 1, 512, 1
...
class KANActivation(torch.nn.Module):
    def __init__(
        ...
        grid_size=8,
        spline_order=3,
        grid_range=[-4.5 * torch.pi, 4.5 * torch.pi],  # 有效区域, 不包括两端扩展区域
        gradient_amplify_factor=10.0,  # 梯度放大倍数
        init_strategy='linear_with_noise',  # 初始化策略
        ...
        self.gradient_amplify_factor = gradient_amplify_factor
    ...
    def _init_linear_with_noise(self):
        ...
        # # 在中间部分添加额外的随机扰动，鼓励更多非线性
        # mid_start = num_coeffs // 4
        # mid_end = 3 * num_coeffs // 4
        # if mid_end > mid_start:
        #     extra_noise = torch.randn(mid_end - mid_start) * noise_amplitude * 0.2
        #     weights[mid_start:mid_end] += extra_noise
    ...
    def _init_random(self):
        ...
        num_coeffs = self.grid_size + self.spline_order
        range_width = self.grid_range[1] - self.grid_range[0]
        return torch.randn(num_coeffs) * range_width * 0.5
    
    def _init_xavier(self):
        """Xavier/Glorot初始化"""
        num_coeffs = self.grid_size + self.spline_order
        # Xavier初始化的标准差
        std = (2.0 / (num_coeffs + 1)) ** 0.5
        weights = torch.randn(num_coeffs) * std
        
        # 添加线性趋势
        linear_trend = torch.linspace(self.grid_range[0], self.grid_range[1], num_coeffs) * 0.1
        return weights + linear_trend
    ...
    def _register_gradient_hook(self):
        ...
        def gradient_amplify_hook(grad):
            """
            梯度放大钩子函数
            在反向传播时自动调用, 对spline_weight的梯度进行放大
            """
            if grad is not None:
                # 可选：打印梯度信息用于调试
                # print(f"Layer {getattr(self, '_layer_info', 'Unknown')} KAN 梯度放大前: {grad.norm():.8f}")
                amplified_grad = grad * self.gradient_amplify_factor
                # print(f"Layer {getattr(self, '_layer_info', 'Unknown')} KAN 梯度放大后: {amplified_grad.norm():.8f}")
